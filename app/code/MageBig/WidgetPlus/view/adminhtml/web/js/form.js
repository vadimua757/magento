...
define([
    ...
    FormElementDependenceController.prototype = {
        ...
        trackChange: function (e, idTo, valuesFrom) {
            ...
            for (idFrom in valuesFrom) { //eslint-disable-line guard-for-in
                ...
                if (from) {
                    ...
                    if (values[0].indexOf("|") != -1) {
                        var varr = values[0].split("|");
                        var count = 0;
                        for (var i = 0; i < varr.length; i++) {
                            count += varr[i].indexOf(from.value);
                        }
                        if (Math.abs(count) < varr.length) {
                            isInArray = true;
                        } else {
                            isInArray = false;
                        }
            ...
            var chooserBlock;
            if (target != undefined && target.hasClassName('bg-input')) {
                chooserBlock = true;
            }
            ...
            if (rowElement == undefined && target) { //eslint-disable-line eqeqeq
                ...
                var levelUp = this._config.levels_up;
                if (chooserBlock) {
                    levelUp = this._config.levels_up + 1;
                }
                rowElement = target.up(levelUp);
